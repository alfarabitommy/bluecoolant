...
class MediaUploadController extends Controller
{
    ...
    public function upload_media_file(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            // dd($image);
            $resize_grid_image = Image::make($folder_path.$image_db)->resize(350, null,function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_large_image = Image::make($folder_path.$image_db)->resize(740, null,function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_thumb_image = Image::make($folder_path.$image_db)->resize(150, 150);
