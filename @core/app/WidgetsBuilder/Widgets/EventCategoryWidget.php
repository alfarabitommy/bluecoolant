...
class EventCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = EventsCategory::where(['lang' => $user_selected_language, 'status' => 'publish'])->orderBy('id', 'DESC')->take($post_items)->get();
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li><a href="' . route('frontend.events.category', ['id' => $post->id,'any' => Str::slug($post->title)]) . '"><i class="fas fa-chevron-right"></i> ' . $post->title . '</a></li>';
        }
        $output .= '</ul>';
